...
(ert-deftest my/noema-workspace-root-follows-desktop-precedence ()
  (let ((process-environment (copy-sequence process-environment)))
    ...
    (setenv "AARONNOTE_ROOT" "/tmp/noema-compat")
    (should (equal (my/noema-workspace-root) "/tmp/noema-primary/"))
    (setenv "NOEMA_ROOT" nil)
    (should (equal (my/noema-workspace-root) "/tmp/noema-compat/"))))

(ert-deftest my/noema-workspace-root-and-layout-use-app-config ()
  (let ((process-environment (copy-sequence process-environment))
        (workspace (make-hash-table :test #'equal)))
    (setenv "NOEMA_ROOT" nil)
    (setenv "AARONNOTE_ROOT" nil)
    (setenv "NOEMA_WORKSPACE_LAYOUT" nil)
    (puthash "root" "~/Documents/Noema-test" workspace)
    (puthash "layout" "wiki" workspace)
    (cl-letf (((symbol-function 'my/noema--workspace-config)
               (lambda () workspace)))
      (should (equal (my/noema-workspace-root)
...
(ert-deftest my/noema-open-wiki-view-uses-canonical-routes ()
  (let (opened)
    (cl-letf (((symbol-function 'my/noema--ensure-server)
              ...
              ((symbol-function 'my/noema--server-url) #'identity)
              ((symbol-function 'my/noema--open-url)
               (lambda (url &rest _) (setq opened url))))
      (my/noema-open-wiki-view "repositories")
  ...
  (let ((my/noema--ready t)
    ...
    (cl-letf (((symbol-function 'my/noema--api-call)
               (lambda (channel args callback)
                 ...
                 (funcall callback '((notes . nil) (generation . "1234567890")
                                     (maintenance . ((mode . "incremental"))))))))
      (my/noema-wiki-refresh)
      (should (equal (car call) "aaronnote:api:wiki:refresh"))
      (should (equal (alist-get 'mode (aref (cadr call) 0)) "incremental")))))

(ert-deftest my/noema-wiki-index-status-renders-repository-hash-tables ()
  (let ((my/noema--ready t)
        (payload (make-hash-table :test #'equal)))
    (puthash "generation" "1234567890" payload)
    (puthash "lastMode" "incremental" payload)
    (puthash "repositories"
             (list (let ((repository (make-hash-table :test #'equal)))
                     (puthash "repositoryId" "private/research" repository)
                     repository))
             payload)
    (cl-letf (((symbol-function 'my/noema-api-call)
               (lambda (_channel _args callback &optional _timeout)
                 (funcall callback payload nil)))
              ((symbol-function 'display-buffer) #'ignore))
      (my/noema-wiki-index-status)
      (with-current-buffer "*Noema Wiki index status*"
        (should (string-match-p "private/research" (buffer-string)))))))
...
(ert-deftest my/noema-jupyter-gateway-routes-session-snapshot-directly ()
  (let (seen deferred)
    (cl-letf (((symbol-function 'my/noema--defer-host-event)
              ...
              ((symbol-function 'my/noema-jupyter-cell-handle-session-event)
...
(ert-deftest my/noema-xwidget-recovery-normalizes-the-shared-key-payload ()
  (let ((plain (my/noema--xwidget-recovery-detail (aref (kbd "d") 0)))
        (space (my/noema--xwidget-recovery-detail (aref (kbd "SPC") 0)))
        (source (my/noema--xwidget-recovery-detail (aref (kbd "M-/") 0)))
        (selection (my/noema--xwidget-recovery-detail (aref (kbd "S-<left>") 0))))
    (should (equal (alist-get 'key plain) "d"))
    (should (equal (alist-get 'code plain) "KeyD"))
    (should (equal (alist-get 'key space) " "))
    (should (equal (alist-get 'code space) "Space"))
    (should (equal (alist-get 'key source) "/"))
    (should (eq (alist-get 'metaKey source) t))
    (should (equal (alist-get 'key selection) "ArrowLeft"))
    (should (eq (alist-get 'shiftKey selection) t))))

(ert-deftest my/noema-xwidget-placeholder-runs-no-buffer-local-plugins ()
  (skip-unless (require 'xwidget nil t))
  (with-temp-buffer
    (xwidget-webkit-mode)
    (let ((my/noema--app-buffer (current-buffer)))
      (setq-local my/noema--client-id "placeholder-client")
      (setq-local completion-at-point-functions '(ignore))
      (setq-local eldoc-documentation-functions '(ignore))
      (setq-local my/xwidget-focus-script "sync-js-must-go")
      ...
      (should-not eldoc-documentation-functions)
      (should-not syntax-propertize-function)
      (should-not my/xwidget-focus-script)
      (should-not cursor-type)
      (should-not bidi-display-reordering)
      (should my/noema-keys-mode)
      (should my/noema-xwidget-recovery-mode)
      (when (boundp 'undo-tree-mode) (should-not undo-tree-mode))
      (dolist (mode minor-mode-list)
        (when (and (local-variable-p mode)
                   (boundp mode)
                   (symbol-value mode)
                   (fboundp mode))
          (should (memq mode my/noema--xwidget-placeholder-mode-whitelist)))))))

(ert-deftest my/noema-xwidget-recovery-keeps-editor-ctrl-keys-in-renderer ()
  "An inert placeholder may own host prefixes, never document Ctrl commands."
  (dolist (key '("C-a" "C-d" "C-r" "C-z" "C-y" "C-0"))
    (should (eq (lookup-key my/noema-xwidget-recovery-mode-map (kbd key))
                #'my/noema-xwidget-recover-key)))
  (dolist (key '("C-x" "C-c" "C-g"))
    (should-not (lookup-key my/noema-xwidget-recovery-mode-map (kbd key))))
  (should (equal (my/noema--xwidget-key-code "=") "Equal"))
  (should (equal (my/noema--xwidget-key-code "-") "Minus")))

(ert-deftest my/noema-xwidget-recovery-never-evaluates-javascript-inline ()
  (let ((buffer (current-buffer))
    ...
    (cl-letf (((symbol-function 'run-at-time)
              ...
              ((symbol-function 'xwidget-webkit-execute-script)
               (lambda (&rest _)
                 (ert-fail "recovery must not evaluate JavaScript"))))
      (my/noema--forward-recovery-key
       buffer '((key . "d") (code . "KeyD")))
      (should (equal sent '("key" ((key . "d") (code . "KeyD")))))
      (setq sent nil)
      (my/noema-xwidget-recover-key (aref (kbd "d") 0))
      (should (eq (car scheduled) #'my/noema--forward-recovery-key))
      (apply (car scheduled) (cdr scheduled))
      (should (equal (car sent) "key")))))

(ert-deftest my/noema-command-notification-leaves-the-caller-before-io ()
  (let ((my/noema--post-queue nil)
        (my/noema--post-timer nil)
    ...
    (cl-letf (((symbol-function 'run-at-time)
               (lambda (_time _repeat function &rest args)
                 (setq scheduled (cons function args))
                 'mock-timer))
              ((symbol-function 'remote-gateway-find-client)
               (lambda (_name) 'client))
              ((symbol-function 'my/noema--post-now)
               (lambda (payload) (push payload sent) t))
              ((symbol-function 'accept-process-output)
               (lambda (&rest _)
                 (ert-fail "notification path must never wait")))
              ((symbol-function 'xwidget-webkit-execute-script)
               (lambda (&rest _)
                 (ert-fail "notification path must never evaluate JS"))))
      (my/noema--post '((type . "command") (command . "focus")))
      (should-not sent)
      (should (= (length my/noema--post-queue) 1))
      (should (eq (car scheduled) #'my/noema--drain-post-queue))
      (apply (car scheduled) (cdr scheduled))
      (should (equal (alist-get 'command (car sent)) "focus"))
      (should-not my/noema--post-queue))))
...
(ert-deftest my/noema-key-event-passes-client-to-run-emacs-key ()
  (let (seen deferred)
    (cl-letf (((symbol-function 'json-serialize)
              ...
              ((symbol-function 'my/noema--defer-host-event)
               (lambda (function &rest args)
                 (setq deferred (cons function args))))
...
(ert-deftest my/noema-ui-state-gateway-does-not-reserialize-status ()
  (let ((my/noema-echo-severity 'error)
    ...
    (cl-letf (((symbol-function 'json-serialize)
              ...
              ((symbol-function 'my/noema--defer-host-event)
               (lambda (function &rest args) (apply function args))))
      (my/noema--gateway-event
       `((type . "ui-state")
    ...
    (unwind-protect
        (save-window-excursion
          ...
          (let ((target-window (split-window-right)))
            ...
            (cl-letf (((symbol-function 'xwidget-webkit-edit-mode)
                       (lambda (arg)
                         (when (= arg 1) (setq edit-buffer (current-buffer)))))
                      ((symbol-function 'my/noema-command)
                       (lambda (command &optional _detail)
                         (when (equal command "focus")
                           (setq focused-buffer (current-buffer))))))
                      ...
                      ((symbol-function 'xwidget-webkit-edit-mode)
                       (lambda (arg)
                         (if (equal arg -1)
                             (setq released-buffer (current-buffer))
                           (when (equal arg 1)
                             (setq armed-buffer (current-buffer))))))
                      ((symbol-function 'my/noema-command)
                       (lambda (command &optional _detail)
                         (when (equal command "focus")
                           (setq focused-buffer (current-buffer))))))
...
(ert-deftest my/noema-runtime-status-renders-debug-payload ()
  (let ((my/noema--ready t)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'my/noema-api-call)
                   (lambda (c a callback &optional _timeout)
                     (setq channel c
                           args a)
...
(ert-deftest my/noema-process-filter-ignores-stale-proc-ready-line ()
  ...
  (let* ((buffer (generate-new-buffer " *Noema-stale-proc-test*"))
         ...
         (my/noema--process-log-timer nil)
         (flush-count 0))
    (unwind-protect
        (cl-letf (((symbol-function 'my/noema--flush-ready-callbacks)
                   (lambda () (cl-incf flush-count))))
...
(ert-deftest my/noema-process-filter-defers-diagnostic-buffer-edits ()
  "Child output must leave the process filter before touching its log buffer."
  (let* ((buffer (generate-new-buffer " *Noema-deferred-log-test*"))
         (proc (make-process
                :name "aaronnote-deferred-log-test"
                :buffer buffer
                :command (list "cat")))
         (my/noema--process-log-queue nil)
         (my/noema--process-log-bytes 0)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'run-at-time)
                  ...
                  ((symbol-function 'timerp)
                   (lambda (timer) (eq timer 'mock-log-timer))))
          (my/noema--process-filter proc "diagnostic line\n")
          (with-current-buffer buffer
            (should (string-empty-p (buffer-string))))
          (should (eq (car scheduled) #'my/noema--flush-process-log))
          (apply (car scheduled) (cdr scheduled))
          (with-current-buffer buffer
            (should (equal (buffer-string) "diagnostic line\n"))))
...
(ert-deftest my/noema-queued-command-keeps-the-issuing-pane-address ()
  "The async payload captures its pane before any later timer can run."
  (let* ((pane (generate-new-buffer " *Noema issuing pane*"))
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'my/noema--ensure-server)
                     (lambda (&rest _)
                       (ert-fail "command enqueue must not start/wait for host")))
