...
(defun my/test--run (command root)
...
(defun my/test-nearest ()
  ...
  (interactive)
  (pcase-let ((`(,root . ,command) (my/test--command 'nearest)))
    (my/test--run command root)))

(defun my/test-file ()
  "Run tests for the current file."
  (interactive)
  (pcase-let ((`(,root . ,command) (my/test--command 'file)))
    (my/test--run command root)))

(defun my/test-project ()
  "Run the project test suite."
  (interactive)
  (pcase-let ((`(,root . ,command) (my/test--command 'project)))
    (my/test--run command root)))
...
(defun my/test-rerun ()
  ...
  (my/test--run my/test-last-command my/test-last-directory))
