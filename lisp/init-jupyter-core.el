...
plain Jupyter language such as \"python\" or \"sage\" and KERNEL is
...
(defvar my/jupyter-language-metadata nil
  "Alist mapping managed language names to integration metadata plists.")

(defvar my/jupyter-current-language-functions nil
  "Abnormal hook used to infer a Jupyter language from the current context.")

(defvar my/jupyter-after-kernelspec-refresh-functions nil
  "Abnormal hook run after Jupyter kernelspec discovery is refreshed.")

(defvar my/jupyter-after-language-connection-file-functions nil
  "Abnormal hook run with LANGUAGE and FILE after remembering a connection file.")
  ...
  (let ((language (my/jupyter--normalize-language language)))
    (member language
            (mapcar #'my/jupyter--normalize-language
                    (append my/jupyter-known-languages
                            (mapcar #'car my/jupyter-language-metadata))))))
  ...
  (let ((language
         (or
          (run-hook-with-args-until-success
           'my/jupyter-current-language-functions)
          (pcase major-mode
...
(defun my/jupyter-default-session-for-language (language fallback)
  "Return the preferred session for LANGUAGE, or FALLBACK."
...
(defun my/jupyter-refresh-kernelspecs-and-reconfigure (&optional quiet)
  "Refresh kernelspecs and reconfigure Babel/Jupyter glue.

When QUIET is non-nil, suppress the final confirmation message."
  (interactive)
  (require 'jupyter-kernelspec)
  (jupyter-refresh-kernelspecs)
  (run-hook-with-args 'my/jupyter-after-kernelspec-refresh-functions)
  (unless quiet
    (message "Refreshed Jupyter kernelspecs and integrations")))
...
(defun my/jupyter-register-language-metadata (language metadata)
  "Register integration METADATA for managed LANGUAGE."
  (setf (alist-get language my/jupyter-language-metadata nil nil #'equal)
        metadata))

(defun my/jupyter-language-metadata (language)
  "Return integration metadata for LANGUAGE, if any."
  (let ((normalized (my/jupyter--normalize-language language)))
    (or (cdr (assoc language my/jupyter-language-metadata))
        (cl-loop for (entry-language . metadata) in my/jupyter-language-metadata
                 when (string= normalized
                               (my/jupyter--normalize-language entry-language))
                 return metadata))))

(defun my/jupyter--metadata-languages ()
  "Return language names supplied by integration metadata."
  (mapcar #'car my/jupyter-language-metadata))

(defun my/jupyter--static-preferred-kernels-for-language (language)
  "Return static preferred kernels configured for LANGUAGE."
  (copy-sequence
   (plist-get (my/jupyter-language-metadata language) :preferred-kernels)))

(defun my/jupyter--managed-languages ()
  "Return all known managed languages in display order."
  (delete-dups
   (append
    (my/jupyter--metadata-languages)
...
(define-derived-mode my/jupyter-manager-mode special-mode "Jupyter-Hub"
  "Major mode for the Jupyter kernel and language management dashboard.")
               ...
               (cl-remove-if-not
                (lambda (language)
                  (string= kernel-language
                           (or (plist-get (my/jupyter-language-metadata language)
                                          :kernel-language)
                               language)))
...
(defun my/jupyter-manager--insert-language-section ()
  ...
  (if-let* ((languages (my/jupyter--managed-languages)))
      (dolist (language languages)
        (let* ((plist (my/jupyter-language-metadata language))
...
(defun my/jupyter-manager--kernel-used-by (kernel language)
  ...
  (cl-remove-if-not
   (lambda (managed-language)
     (and (string= language
                   (or (plist-get (my/jupyter-language-metadata managed-language)
                                  :kernel-language)
                       managed-language))
          (string= kernel
...
(defun my/jupyter-manager ()
  "Open the Jupyter kernel and language management dashboard."
  (interactive)
...
(defun my/jupyter--remember-connection-file (language file)
  ...
  (dolist (alias (my/jupyter--language-family language))
    (setf (alist-get alias my/jupyter-language-connection-files nil 'remove #'equal)
          file)
    (run-hook-with-args
     'my/jupyter-after-language-connection-file-functions
     alias
     file))
  file)
...
incorrectly, producing `void-variable state' whenever REPL setup or
kernel execution touches them.  Rebinding them here keeps the package usable
