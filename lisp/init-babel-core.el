;;; init-babel-core.el --- Org Babel configuration centered on ob-jupyter -*- lexical-binding: t -*-
...
(declare-function my/jupyter-language-metadata "init-jupyter" (language))
(declare-function my/jupyter-preferred-kernels-for-language "init-jupyter" (language fallback))
(declare-function my/jupyter-read-connection-file "init-jupyter" (&optional prompt language))
(declare-function my/jupyter-register-language-connection-file "init-jupyter"
                  (language file &optional quiet))
(declare-function my/jupyter-register-language-metadata "init-jupyter" (language metadata))
...
(defvar my/jupyter-after-kernelspec-refresh-functions)
(defvar my/jupyter-after-language-connection-file-functions)
(defvar my/jupyter-current-language-functions)
(defvar my/jupyter-manager-extra-section-functions)
...
(dolist (entry my/org-babel-jupyter-native-backends)
  (my/jupyter-register-language-metadata (car entry) (cdr entry)))
...
(defun my/org-babel--async-eval-form (source-file point default-dir output-file)
  ...
  (prin1-to-string
   `(progn
      ...
      (load-file ,(expand-file-name "lisp/init-babel.el" my/org-babel-config-root))
...
(defun my/org-babel-jupyter-current-language ()
  "Return the current Org source block language for Jupyter, or nil."
  (when (and (derived-mode-p 'org-mode)
             (fboundp 'org-in-src-block-p)
             (ignore-errors (org-in-src-block-p t)))
    (car (org-babel-get-src-block-info 'light))))

(defun my/org-babel-jupyter-sync-language-session (language file)
  "Set LANGUAGE's Jupyter source block session default to FILE."
  (my/org-babel-jupyter-set-language-session language file "yes"))

(defun my/org-babel--loaded-languages ()
...
(defun my/org-babel--src-mode-for-language (language)
...
(defun my/org-babel-jupyter--alias-entries ()
...
(add-hook 'my/jupyter-current-language-functions
          #'my/org-babel-jupyter-current-language)
(add-hook 'my/jupyter-after-kernelspec-refresh-functions
          #'my/org-babel-configure-jupyter-backends)
(add-hook 'my/jupyter-after-language-connection-file-functions
          #'my/org-babel-jupyter-sync-language-session)
(add-hook 'my/jupyter-manager-extra-section-functions
          #'my/org-babel-jupyter-manager-insert-section)
(global-set-key (kbd "C-c j o") #'my/jupyter-use-connection-file-for-org-block)
...
(provide 'init-babel-core)
;;; init-babel-core.el ends here
