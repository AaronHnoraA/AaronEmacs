...
function hiddenScriptPath(noteFile, kernel, session, language) {
  ...
  const safeKernel = safeSlug(kernel, "python3");
  const safeSession = safeSlug(session, "default");
  const ext = extensionForLanguage(language);
  return join(dirname(noteFile), `.${noteBase}.${safeLanguage}.${safeKernel}.${safeSession}.${ext}`);
}

function outputMirrorPath(noteFile, kernel, session) {
  const noteExt = extname(noteFile);
  const noteBase = safeSlug(basename(noteFile, noteExt), "note");
  const safeKernel = safeSlug(kernel, "python3");
  const safeSession = safeSlug(session, "default");
  return join(dirname(noteFile), `.${noteBase}.cell-output.${safeKernel}.${safeSession}.json`);
}
...
export function createJupyterCellService({
  ...
  const runScript = join(jupyterRoot, "scripts", "run-jupyter-lab.sh");
  ...
  async function kernels() {
    ...
    return {
      ...
      kernels: Object.entries(specs).map(([name, spec]) => ({
        name,
  ...
  async function openScript(body) {
    ...
    const scriptFile = hiddenScriptPath(noteFile, kernel, session, language);
    ...
    const payload = { file: scriptFile, line: rendered.line, col: 0 };
  ...
  async function readScriptCell(body) {
    ...
    const scriptFile = hiddenScriptPath(noteFile, kernel, session, language);
    const outputFile = outputMirrorPath(noteFile, kernel, session);
  ...
  async function executeScriptCell(body) {
    ...
    const outputFile = outputMirrorPath(noteFile, read.kernel, read.session);
  ...
  return { execute, kernels, openScript, readScriptCell, executeScriptCell, clearScriptCellOutput, restart, interrupt, shutdown };
}
