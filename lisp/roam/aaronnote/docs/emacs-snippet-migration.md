...
Aaronnote and Emacs share the YAS files under `snippets/`. Aaronnote reads those
...
Aaronnote supports ordinary text, `$0`, numeric fields, defaults, nested
...
and omitted from Aaronnote completion.

Generated metadata uses YAS's native `uuid` and `contributor` headers. The
URL-encoded contributor suffix is decoded only by Aaronnote, so YAS loads the
same files without unknown-directive warnings:

```snippet
# uuid: latex-workshop:@a
# contributor: Aaronnote provider=latex-workshop&priority=180&weight=1.2&context=math-at&description=alpha
```
