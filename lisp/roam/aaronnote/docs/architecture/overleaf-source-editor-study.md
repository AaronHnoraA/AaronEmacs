# Overleaf Source Editor 架构审读与 Aaronnote 迁移矩阵

审读基线：Overleaf 官方仓库提交
`28ad3b03b71cb4311decdcb55c36b33ec10d72db`（2026-06-26）。

本文件记录一次性架构迁移的依据和边界。Aaronnote 与 Overleaf 的产品定位不同：
Aaronnote 继续以 Markdown 为唯一真相源，不引入 LaTeX Visual Editor、多人 OT、
编译/审阅业务或 Overleaf 主题。直接采用的 Overleaf 源码按 AGPL-3.0 分发；
改造后的 Aaronnote 不建立自动上游同步机制。
...
- 标准 Markdown 继续使用 Lezer Markdown；Aaronnote link extension 归入
  `languages/markdown`。
...
| 内容 | 原因 |
|---|---|
| `lezer-latex`、LaTeX grammar/token | Aaronnote 坚持 Markdown；解析成本和扩展方向不符 |
| LaTeX atomic/mark widget 业务分支 | 对应命令和 environment 不属于 Aaronnote 语义 |
| React/Angular context shell | Aaronnote 是轻量原生 TS + Emacs 宿主，不为结构迁移引入 UI 框架 |
| realtime OT、track changes、review、compile | 产品功能不在范围内 |
| Overleaf visual theme | Aaronnote 主题、proof/自定义块外观必须保持 |
| 初次显示前强制全文解析 | 大文档性能退化；Aaronnote 只等待 viewport + overscan |
| atomic/mark decoration 全量重建 TODO | Overleaf 源码仍有未完成增量化；Aaronnote 采用 projection 修补 |
| 动态第三方插件运行时 | Aaronnote 已移除此能力；仅使用静态内部 feature registry |
...
- Markdown 文本、保存协议、API channel、SSE command、Editor facade 与 Emacs
  公开命令保持不变。
- Aaronnote 只接管 `.md`、`.markdown` 和 README；`.tex` 恢复原 Emacs 打开流程。
