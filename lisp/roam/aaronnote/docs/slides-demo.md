#+begin meta
title: Aaronnote × Reveal stress-test deck
date: 2026-07-12
kind: slides
...
同一个 Markdown 文件，两种视图：`Cmd+/` 在 Reveal 和普通 Aaronnote 笔记之间切换。
...
公式、org-env、表格和 Aaronnote 后续升级都走普通笔记的 HTML renderer。
#+end note
...
下面不是代码块，而是经过 Aaronnote sanitizer 后的真实 HTML：
...
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:18px;text-align:left">
  ...
  <div style="padding:18px;border:1px solid #4a6075;border-radius:12px"><strong>Aaronnote</strong><br>公式与 widgets</div>
...
The code block, surrounding prose and formula \(\max_i|p_{i+1}-p_i|\) are all rendered by Aaronnote.
...
1. 退出 Reveal，回到完整、连续的 Aaronnote 所见即所得笔记；
