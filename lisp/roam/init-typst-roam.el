...
(defcustom my/typst-roam-recent-limit 24
  "Maximum number of recent Typst roam notes kept in memory."
  :type 'integer
  :group 'my/typst-roam)

(defcustom my/typst-roam-select-window-height 0.32
  "Height for the bottom Typst roam selector window."
  :type '(choice (number :tag "Fraction or rows")
                 (function :tag "Window height function"))
  :group 'my/typst-roam)

(defvar my/typst-roam--recent nil
  "Recently opened Typst roam note slugs, newest first.")
...
(defun my/typst-roam--target-at-point ()
  "Return the raw note-link target at or near point, or nil."
  (save-excursion
    ...
    (when (file-exists-p file)
      (with-temp-buffer
        ...
        (let (parts)
          (goto-char (point-min))
          (while (and (not (eobp))
                      (< (length (string-join (nreverse parts) " ")) 220))
            (let ((line (string-trim
                         (buffer-substring-no-properties
                          (line-beginning-position)
                          (line-end-position)))))
              (unless (or (string-empty-p line)
                          (string-match-p "\\`#\\(import\\|show\\|set\\|roam-meta\\)" line)
...
(defun my/typst-roam--heading-labels (&optional file)
  "Return Typst tag ids/labels in FILE or the current buffer.
...
(defun my/typst-roam--goto-tag-id (id)
  "Jump to Typst label/tag ID in the current buffer."
  (goto-char (point-min))
...
(defun my/typst-roam-insert-link ()
  "Open the interactive selector and insert a #note-link."
  (interactive)
...
(defun my/typst-roam-new-node (&optional title directory)
  "Create a new timestamped Typst roam node from TITLE in DIRECTORY."
  (interactive)
...
(defun my/typst-roam--note-file-from-fields (key note)
  ...
  (let* ((root (my/typst-roam-root))
         (raw (or (my/typst-roam--note-field note "file")
                  (my/typst-roam--note-field note "path")
                  (my/typst-roam--note-field note "link")
                  (my/typst-roam--note-field note "source")
                  key))
...
(defun my/typst-roam--slugify-tag-id (text)
  "Return a stable Typst label id for TEXT."
...
(defun my/typst-roam-generate-tag-id (&optional text)
  "Generate a unique Typst tag id from TEXT or context."
  (interactive)
  (let* ((source (or text
                     (when (use-region-p)
                       (buffer-substring-no-properties
...
(defun my/typst-roam-insert-tag-id (&optional id)
  "Insert or append Typst label/tag ID at point.
On a heading line, append `<id>` unless a label already exists."
  (interactive)
  (let ((id (or id
                (read-string "Tag id: "
    ...
    (save-excursion
      (beginning-of-line)
      (if (looking-at "^=+\\s-+.+?\\(?:\\s-*<\\([[:alnum:]_:-]+\\)>\\)?\\s-*$")
          (if (match-string 1)
              (user-error "Heading already has tag id: %s" (match-string 1))
            (end-of-line)
            (insert (format " <%s>" id)))
        (insert (format "<%s>" id))))))
        ...
        (if (null todos)
          ...
          (dolist (entry todos)
            (let* ((note-slug (gethash "note" entry))
              ...
              (insert-text-button
               (format "  [%s]  %s\n" title text)
               'action (lambda (_) (my/typst-roam--open-slug note-slug))
               'follow-link t))))
        (goto-char (point-min))
        (special-mode)))
    (display-buffer buf)))
    ...
    (if (called-interactively-p 'interactive)
        (if (= (length entries) 1)
          ...
          (my/typst-roam--show-note-list
           (format "Typst roam search: %s" query)
           entries
           "(no matching notes)"))
      entries)))
...
(defun my/typst-roam-graph ()
  ...
  (let* ((entries (my/typst-roam--all-note-summaries))
    ...
    (with-current-buffer buf
      (let ((inhibit-read-only t))
        (erase-buffer)
        (insert (format "Typst roam graph: %d nodes\n\n" (length entries)))
        (dolist (entry entries)
          (let ((slug (plist-get entry :slug))
...
;; ── Interactive note-link selector ───────────────────────────────────────────
...
(define-derived-mode my/typst-roam-select-mode special-mode "Roam-Select"
  "Interactive Typst roam note-link selector."
...
(defun my/typst-roam--target-suffix (kind target)
  "Return note-link suffix for KIND and TARGET."
  (pcase kind
    ('tag (concat "#" (my/typst-roam--encode-ref target)))
    ('dom (concat "@" (mapconcat #'my/typst-roam--encode-ref
                                  (my/typst-roam--dom-target-segments target)
...
(defun my/typst-roam--link-target-for-record (record basis &optional kind target)
  "Return note-link target for RECORD using BASIS, KIND, and TARGET."
...
(defun my/typst-roam--insert-note-link-target (target text &optional marker)
  "Insert #note-link TARGET with TEXT at MARKER or point."
  (let ((link (format "#note-link(\"%s\")[%s]" target text)))
...
(defun my/typst-roam-copy-link-to-here ()
  "Copy a #note-link to the current note or current heading.
When point is on a heading, ensure a Typst `<tag-id>` exists and copy a
...
(defvar my/typst-roam-override-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c r") my/typst-roam-map)
    (define-key map (kbd "C-c C-o") #'my/typst-roam-follow-link)
    map)
  "High-priority Typst roam keymap used above LSP minor-mode maps.")

(define-minor-mode my/typst-roam-override-mode
  "Give Typst roam keys priority over LSP minor-mode keymaps."
  :init-value nil
  :lighter nil
  :keymap my/typst-roam-override-mode-map)

(defvar my/typst-roam--emulation-mode-map-alist
  `((my/typst-roam-override-mode . ,my/typst-roam-override-mode-map))
  "Emulation map alist for high-priority Typst roam keys.")

(add-to-list 'emulation-mode-map-alists
             'my/typst-roam--emulation-mode-map-alist)

(with-eval-after-load 'evil
  (evil-define-key* 'normal my/typst-roam-override-mode-map
    (kbd "gd") #'my/typst-roam-goto-definition))
...
(defun my/typst-roam-xref-backend ()
  "Use typst-roam as xref backend when point is on a #note-link target."
    ...
    (let ((pos (cond
                ((plist-get parsed :id)
                 ...
                 (when (re-search-forward
                        (format "<%s>" (regexp-quote (plist-get parsed :id)))
                        nil t)
                   (match-beginning 0)))
                ((plist-get parsed :dom)
                 (plist-get
      ...
      (run-with-idle-timer
       ...
       (lambda ()
         (when (buffer-live-p buf)
           (with-current-buffer buf
             (when-let* ((target (my/typst-roam--target-at-point))
                         (parsed (my/typst-roam--parse-target target))
                         (note-id (plist-get parsed :slug))
                         (file (plist-get parsed :file))
                         ((file-exists-p file)))
