...
export function createEditor(
  ...
  return {
    ...
    insertText(text: string, deleteBefore = 0): void {
      if (inSource) {
        ...
        view.dispatch(
          view.state.tr
            .insertText(text, Math.max(0, from - deleteBefore), to)
            .scrollIntoView(),
        );
        view.focus();
      }
    ...
    cursorContext(maxChars = 500) {
      if (inSource) {
        ...
        const rect = (() => {
          const y = caretYInTextarea(pos);
          if (y == null) return null;
          const box = sourceTextarea.getBoundingClientRect();
          return { left: box.left + 24, top: y, bottom: y + 18 };
        })();
        return {
