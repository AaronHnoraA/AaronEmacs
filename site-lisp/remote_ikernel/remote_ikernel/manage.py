...
AARON_METADATA_KEY = 'aaron'
REMOTE_KERNEL_METADATA_KEY = 'remote_kernel'
REMOTE_KERNEL_GROUPS = ('core', 'temporary')
...
def set_kernel_group(kernel_name, group):
    """Set Aaron's persistent management group for KERNEL_NAME."""
    if group not in REMOTE_KERNEL_GROUPS:
        raise ValueError("Unknown remote kernel group: {0}".format(group))
    spec = ks.get_kernel_spec(kernel_name)
    kernel_file = path.join(spec.resource_dir, 'kernel.json')
    with open(kernel_file) as stream:
        kernel_json = json.load(stream)

    metadata = kernel_json.setdefault('metadata', {})
    aaron = metadata.setdefault(AARON_METADATA_KEY, {})
    remote = aaron.setdefault(REMOTE_KERNEL_METADATA_KEY, {})
    remote['group'] = group

    temporary = kernel_file + '.tmp'
    with open(temporary, 'w') as stream:
        json.dump(kernel_json, stream, sort_keys=True, indent=2)
        stream.write('\n')
    os.rename(temporary, kernel_file)
...
def add_kernel(interface, name, kernel_cmd, cpus=1, pe=None, language=None,
               ...
               launch_cmd=None, group='temporary'):
    """
    ...
    configured_kernel_cmd = kernel_cmd
    ...
    kernel_json = {
        ...
        'metadata': {
            AARON_METADATA_KEY: {
                REMOTE_KERNEL_METADATA_KEY: {
                    'group': group,
                    'config': {
                        'interface': interface,
                        'name': name,
                        'kernel_cmd': configured_kernel_cmd,
                        'cpus': cpus,
                        'pe': pe,
                        'language': language,
                        'system': system,
                        'workdir': workdir,
                        'host': host,
                        'remote_precmd': precmd,
                        'remote_launch_args': launch_args,
...
def manage():
    ...
    parser.add_argument('--set-group', nargs=2,
                        metavar=('KERNEL', 'GROUP'),
                        help="Set a managed kernel group to core or temporary.")
    parser.add_argument('--group', choices=REMOTE_KERNEL_GROUPS,
                        default='temporary',
                        help="Management group for a newly added kernel "
                        "(default: temporary).")
    ...
    if args.add:
        kernel_name, display_name = add_kernel(
                ...
                args.verbose, args.launch_cmd, args.group)
        print("Added kernel ['{0}']: {1}.".format(kernel_name, display_name))
    elif args.set_group:
        kernel_name, group = args.set_group
        if kernel_name not in existing_kernels:
            print("Could not find: {0}.".format(kernel_name))
            raise SystemExit(1)
        if group not in REMOTE_KERNEL_GROUPS:
            print("Unknown remote kernel group: {0}.".format(group))
            raise SystemExit(2)
        set_kernel_group(kernel_name, group)
        print("Set kernel ['{0}'] group to {1}.".format(kernel_name, group))
    elif args.delete:
        undeleted = []
