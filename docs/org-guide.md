...
`H-o w` 里的 TODO、tags、schedule、deadline、priority、property、refile、archive
和 clock 都是当前 heading 的元数据；`T` / `U` 才是在当前位置插入 timestamp。
...
- `H-o g`
  清理 GeoGebra 导出的 TikZ：region / 当前 Org block / kill ring，生成折叠的 raw block 和 `display_latex` 预览块
- `H-o G`
  展开/折叠命令生成的 GeoGebra raw TikZ block
- `H-o o`
...
- `H-o T` / `H-o P` / `H-o v` / `H-o V`
  刷新 overview TOC / 打开左侧 HTML Previewer 工作台 / 手动预览当前可视区域 LaTeX / 预览当前 `display_latex` 块
- `H-o w`
...
- 文件名带 `keep-` 前缀时豁免清理；`H-o k` 可以给当前链接文件加这个前缀
...
- `olivetti`
- RaTeX 光标附近公式编辑预览
- HTML Previewer 工作台（Org / Markdown / HTML 自动打开）

现在默认走低功耗写作模型：Org buffer 尽量接近 stock font-lock，主要视觉渲染交给 Previewer 的 HTML/CSS/JS。`org-modern`、`org-appear`、`valign`、`olivetti`、special block overlay 和可见区 LaTeX overlay 默认不挂 Org source buffer。
...
- `SPC m t`
  todo
- `SPC m P`
  打开 HTML Previewer / Org 编辑工作台；宽窗口左右分屏，窄窗口上下分屏
  Org 光标/窗口位置会单向带动 Previewer；预览窗滚动不会反向移动 Org，预览里 `M-左键` 可跳回源码位置
- `SPC m v`
  手动预览当前可视区域 LaTeX
- `SPC m z`
  从 BibTeX block 填充 Zotero 模板字段
- `SPC m m g`
  清理 GeoGebra 导出的 TikZ，并折叠保留原始导出块
- `SPC m m f` / `SPC m m F`
  展开/折叠当前命令生成的 GeoGebra raw block / 折叠当前 buffer 里的全部 generated raw blocks
- `SPC m m t` / `SPC m m d`
  插入 `tikzpicture` / `tikzcd` 的 `display_latex` 块
- `SPC m m p`
  预览当前 `display_latex` 块
...
- `tikzpic`
  插入 `display_latex` 包裹的 `tikzpicture`
- `tikzcd`
  插入 `display_latex` 包裹的交换图骨架
- `tikznode` / `tikzdraw` / `tikzpath`
  插入常用 TikZ node、draw、path 片段
- `ggbraw`
  插入可被 `my/org-tikz-fold-generated-raw-blocks` 识别的 GeoGebra raw TikZ block
...
- Org 里启用了 `cdlatex`
- 默认公式视觉预览放到 HTML Previewer：`SPC m P` / `H-o P` 打开 xwidget 预览，宽窗口预览在左、窄窗口预览在上，Org buffer 负责输入
- Previewer 公式渲染优先使用本地 vendored KaTeX，失败时交给本地 MathJax 兜底；本地渲染资源超过 21 天会在打开 Previewer 后后台尝试刷新
- Emacs 编辑区保留 RaTeX 的光标附近 posframe 预览
- 自定义 `xelatex + dvisvgm` 异步 SVG overlay 管线保留为手动 fallback；默认不再随滚动自动渲染可见区域
- `tikzpicture`、`tikzcd`、`pgfplots` 和 `quantikz` 只在当前预览片段需要时临时加预览 preamble，不拖慢普通公式预览
- `org-fragtog` 已不再默认常驻；公式进出编辑状态由自定义异步预览管线处理
- AUCTeX 和 `pdf-tools` 已配置
- AUCTeX 支持 `latexmk -pvc` 实时编译和 Sioyek 双向同步
- GeoGebra 导出的 TikZ 可以用 `H-o g` / `SPC m m g` 清理成较短的 `tikzpicture`，原始导出会保存在 `#+begin_src latex :exports none` raw block 里，并用轻量 overlay 折叠
