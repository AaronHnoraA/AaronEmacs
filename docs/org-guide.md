...
现在默认走低功耗写作模型：Org buffer 保留轻量可读性增强，主要视觉渲染交给 Previewer 的 HTML/CSS/JS。`valign` 只在 buffer 已有表格或新插入表格时打开，不再为了发现可见表格挂滚动检测；`org-appear` 在 point/编辑状态未变化时不重复解析。自定义 special block overlay 和可见区 LaTeX overlay 仍可手动或通过配置恢复，但默认不再随滚动自动重绘。
